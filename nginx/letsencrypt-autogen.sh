...
if [ "$LORC_SSL" = "NO" ] || [ ! -z "$LORC_SSL_KEY" ]; then
...
if [ -e "letsencrypt/live/$DOMAIN" ]; then
	...
	echo -e "\033[91mWARN: We couldn't automatically generate the SSL certificate. Review the console log. LiveORC will likely be inaccessible.\033[39m"
fi
